...
/**
 * Detached row. Index access is zero-based; duplicate labels resolve to the
 * first column.
 */
public final class SqlRow {
	...
	SqlRow(ResultSet result) throws SQLException {
		...
		for (int i = 0; i < values.length; i++) {
			...
			values[i] = detach(result.getObject(i + 1));
		}
		...
		if (value instanceof Blob) {
			...
			try {
				...
				if (length > Integer.MAX_VALUE)
					throw new SQLException("LOB too large for a detached row; use forEach/RowMapper streaming");
