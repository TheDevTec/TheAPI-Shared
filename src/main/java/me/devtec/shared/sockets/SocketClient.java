				...
				try {
					...
					try {
						...
						if (e.getMessage() != null
								&& "Connection refused: no further information".equals(e.getMessage())) {
							...
							while (--tries > 0) {
								...
								try {
									...
									if (r.getMessage() != null
											&& "Connection refused: no further information".equals(r.getMessage())) {
								...
								try {
									...
									while (keyIterator.hasNext()) {
										...
										if (key.isWritable() && socketPhase == SocketPhase.LOGIN && !providedLogin) {
											try (Config settings = new Config().set("name", clientName).set("password",
													password)) {
												write(settings);
											}
											providedLogin = true;
										...
										if (key.isReadable()) {
											...
											Map<String, Object> data = (Map<String, Object>) Json.reader()
													.read(container.toString());
									...
									if (e.getMessage() != null && ("Connection reset by peer".equals(e.getMessage())
											|| "Connection refused: no further information".equals(e.getMessage())
