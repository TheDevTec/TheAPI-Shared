...
public class MergeStandards {
	public static final MergeSetting ADD_MISSING_HEADER = new MergeSetting() {
		...
		public boolean merge(Config config, Config merge) {
			...
			try {
				...
				if (config.getDataLoader().getHeader() != null && merge.getDataLoader().getHeader() != null
						&& !merge.getDataLoader().getHeader().isEmpty() && config.getDataLoader().getHeader().isEmpty()
						&& (itr = config.getDataLoader().entrySet().iterator()).hasNext()
						&& itr.next().getValue().comments == null) {
	...
	public static final MergeSetting ADD_MISSING_FOOTER = new MergeSetting() {
		...
		public boolean merge(Config config, Config merge) {
			...
			boolean change = false;
			try {
				if (config.getDataLoader().getFooter() != null && merge.getDataLoader().getFooter() != null
						&& !merge.getDataLoader().getFooter().isEmpty()
						&& config.getDataLoader().getFooter().isEmpty()) {
	...
	public static final MergeSetting[] DEFAULT = { ADD_MISSING_KEYS, ADD_MISSING_HEADER, ADD_MISSING_FOOTER,
				...
				try {
					...
					loop: while (iterator.hasNext()) {
						...
						for (String section : sections)
							if (key.getKey().startsWith(section) && (key.getKey().length() == section.length()
									|| key.getKey().charAt(section.length()) == '.'))
								continue loop;
						change |= mergeDataValue(config, key);
					}
				} catch (Exception ignored) {
	...
	private static boolean mergeDataValue(Config config, Entry<String, DataValue> key) {
		...
		if (configVal == null || configVal.value == null) {
			...
			config.getDataLoader().set(key.getKey(), configVal);
			return true;
		}
		return false;
	}
